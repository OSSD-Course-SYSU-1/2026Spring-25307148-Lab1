...
1. **资源限定符适配**：新增 `resources/tablet/element/float.json`，为平板提供更大的按钮和文字尺寸，代码中统一使用 `$r('app.float.xxx')`，无需硬编码。
2. **响应式布局**：主页采用百分比宽度和 `Scroll` 容器，确保按钮列表在小屏手机和大屏平板上都能完整显示。
3. **动态高度计算**：冒泡排序可视化页面的柱状图高度改为根据屏幕可用高度动态计算，避免平板显示过小。
4. **未来扩展**：可结合断点系统，在平板上实现左右分栏布局（算法列表 + 可视化区域），进一步提升大屏体验。

## 多端部署与自由流转的简单实现

### 多端部署
- 使用**资源限定符**机制，在 `resources/tablet/element/float.json` 中定义平板专属尺寸。
- 代码中所有按钮、文字尺寸均通过 `$r('app.float.xxx')` 引用，系统根据当前设备自动选择正确的资源值。
- 无需修改任何 `.ets` 业务代码，真正实现“一次开发，多端部署”。

### 自由流转
- 在 `module.json5` 中为 `EntryAbility` 设置 `continuable: true`，开启应用接续。
- 应用接续流程设计：
  1. 用户在手机端进入冒泡排序可视化页面，生成随机数组并开始排序动画。
  2. 用户通过系统流转入口，将任务迁移到平板或智慧屏。
  3. 系统触发 `EntryAbility.onContinue()`，保存当前数组数据与页面路径到 `wantParam`。
  4. 目标设备上 `EntryAbility.onCreate()` 接收参数，通过 `AppStorage` 传递给页面，页面自动恢复数组和排序状态。
  5. 用户在目标设备上无缝继续观看排序过程。
- 此功能基于 HarmonyOS 分布式能力，真正实现“自由流转”。